...
class WorkSearching:
    ...
    def __salary_parsing(salary: str) -> dict:
        ...
        if len(tmp_digits) == 2:
            dict_result['Min'] = float(tmp_digits[0])
            dict_result['Max'] = float(tmp_digits[1])
            dict_result['Valuta'] = re.search(r'[a-zA-Zа-яА-ЯеЁ]+$', tmp).group()
        # маска: sum...valuta
        elif re.search(r'([оО][тТ])|([дД][оО])', tmp) is None:
            dict_result['Min'] = float(tmp_digits[0])
            dict_result['Max'] = float(tmp_digits[0])
            dict_result['Valuta'] = re.search(r'[a-zA-Zа-яА-ЯеЁ]+$', tmp).group()
        # маска: от...sum...valuta
        ...
        if WorkSearching.__list_sites[self.__current_site_number][0] == 'HeadHunter':
            ...
            while True:
                ...
                for vacancy in vacancies:
                    ...
                    link_tmp = self.__link_complete(name_info['href'])
                    link_tmp = link_tmp.split('?')
                    dict_vacancy['Link'] = link_tmp[0]
                    ...
                    if salary is None:
                        dict_vacancy['Salary'] = {}
                        dict_vacancy['Salary']['Min'] = None
                        dict_vacancy['Salary']['Max'] = None
                        dict_vacancy['Salary']['Valuta'] = None
                    else:
            ...
            numb = parsed_response.find('span', {'class': '_1ZlLP'})
            print(f'Всего найдено вакансий  на {self.__list_sites[self.__current_site_number][0]}: {numb.text}')

            # Подготовка информации о классах, по которым будет осуществляться поиск.

            # класс тэга div, содержащего информацию о вакансии
            div_class_item = 'Fo44F'
            ...
            div_class_name = '_3LJqf'

            span_class_salary = '_2Wp8I'  # класс тэга span с з/п
            ...
            span_class_company = '_3Fsn4'

            # Информация по кнопке "дальше"
            a_class_button_next = 'f-test-link-Dalshe'    # кнопка дальше
    ...
    def df_adding(self, ind: int, dv: dict) -> None:
        ...
        :return: None
        """
        min_value = dv['Salary']['Min'] if dv['Salary']['Min'] is not None else numpy.NaN
        max_value = dv['Salary']['Max'] if dv['Salary']['Max'] is not None else numpy.NaN
        valuta_value = dv['Salary']['Valuta'] if dv['Salary']['Valuta'] is not None else 'not specified'
    ...
    def save_db(self, db_name_param: str = None) -> None:
        """
        Записывает имеющиеся данные в БД.
        :param db_name_param: имя базы данных, куда будут записаны данные
        :return: None
        """
        if len(self.list_result) == 0:
            raise ValueError('ERROR. Empty vacancy list.')

        result = DB.data_insert('vacancies', self.list_result)

        print(f'\nВсего документов для добавления в БД: {len(self.list_result)}.')
        print(f'Всего документов было добавлено в БД: {result["valid_records_count"]}')
        print(f'Количество документов, которые не получилось добавить в БД: {result["invalid_records"]["count"]}\n')

        if result['invalid_records']['count'] != 0:
            print("Какие действия произвести с документами, которые не получилось добавить:")
            print("1. Вывести на экран (введите 1).")
            print("2. Вывести в файл (введите 2).")
            print("Нажмите любую другую клавишу, чтобы завершить работу с добавлением данных.")

            choise = input("Выберите вариант действий: ")
            if choise == '1':
                for item in result['invalid_records']['records']:
                    pprint(item)
            elif choise == '2':
                file_name = f'{os.getcwd()}\\invalid_vacancies_{len(result["invalid_records"]["records"])}.json'
                with open(file_name, 'w', encoding='utf-8') as f:
                    json.dump(result['invalid_records']['records'], f, ensure_ascii=False, indent=4)
                print(f'Документы, которые не были добавлены в БД, были записаны в файл {file_name}.\n')
        return
    ...
    while True:
        ...
        while True:
            try:
                ...
                print("3. Сохранить результат в БД")
                print("4. Завершить работу с полученными данными")
                ...
                if result_action == 1:
                    ...
                    db_name = input("Если требуется, то введите наименование БД для записи: ")
                    if db_name != '':
                        searching_query.save_db(db_name)
                    else:
                        searching_query.save_db()
                    print(f'Работа по добавлению документов в базу данных завершена.\n')
                elif result_action == 4:
                    print('*' * 20)
                    print()
                    break
                else:
